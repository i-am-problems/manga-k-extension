...
var _Sources = (() => {
  ...
  var extractNextData = ($2) => {
    const raw = $2("#__NEXT_DATA__").html() || "{}";
    try {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const tagSections = [];
    if (genres.length > 0) {
      const tags = genres.map((g) => App.createTag({ id: g.slug, label: g.name }));
    ...
    return App.createSourceManga({
      ...
      mangaInfo: App.createMangaInfo({
        ...
        artist: manga.artist ?? "",
        tags: tagSections,
        desc: decode(manga.summary ?? "")
      })
    });
  };
  var parseInternalMangaId = ($2) => {
    const pageProps = extractNextData($2);
    const manga = pageProps["initialManga"];
    const id = manga?.id;
  ...
  var parseChapters = (apiJson, mangaId) => {
    ...
    const chapters = list.map((ch, index2) => {
      const name = ch.name ?? "";
      const match = name.match(/(\d+(?:\.\d+)?)/);
      const chapNum = match && match[1] !== void 0 ? Number(match[1]) : ch.chapter_number ?? index2 + 1;
      return App.createChapter({
        id: ch.slug,
        name,
        chapNum,
        time: ch.updated_at ? new Date(ch.updated_at) : /* @__PURE__ */ new Date(),
  ...
  var parseChapterPages = ($2) => {
    const pageProps = extractNextData($2);
    const initialChapter = pageProps["initialChapter"];
  ...
  var parseSearch = (apiJson) => {
    if (!apiJson.success || !apiJson.data?.items) {
      return { results: [], hasNext: false };
    }
    const results = apiJson.data.items.filter((item) => item.slug).map((item) => App.createPartialSourceManga({
      mangaId: item.slug,
      image: item.cover ?? "",
  ...
  var parsePopular = ($2) => {
    const items = [];
    const elements = $2(".top-item").length ? $2(".top-item") : $2("article.group, .group");
    elements.each((_, element) => {
      const unit = $2(element);
      const image = unit.find("img").first().attr("data-src") || unit.find("img").first().attr("src") || "";
      const href = unit.find("a[aria-label]").first().attr("href") || unit.find("a[title]").first().attr("href") || unit.find("a").first().attr("href") || "";
      const mangaId = normalizeMangaId(href);
      const title = unit.find("a[title]").first().attr("title") || unit.find(".meta .title a").text().trim() || unit.find("a > span").first().text().trim() || unit.find("img").first().attr("alt") || "";
      const latestChapter = unit.find('a[href*="/chapter"]').first().text().trim() || unit.find(".thumb .latest-chapter").text().trim() || "";
      const chapterMatch = latestChapter.match(/Chapter\s*([0-9]+(?:\.[0-9]+)?)/i);
      const subtitle = chapterMatch && chapterMatch[1] !== void 0 ? `Ch. ${chapterMatch[1]}` : latestChapter;
      if (title && mangaId) {
  ...
  var parseLatest = ($2) => {
    const pageProps = extractNextData($2);
    const rawItems = pageProps["items"] ?? [];
    const pagination = pageProps["pagination"];
    const results = rawItems.filter((item) => item.slug).map((item) => App.createPartialSourceManga({
      mangaId: item.slug,
      image: item.cover ?? "",
      title: decode(item.name ?? ""),
  ...
  var parseHomeSections = (popular, latest, sectionCallback) => {
  ...
  var MangakInfo = {
    ...
    sourceTags: [
      { text: "Cloudflare", type: import_types3.BadgeColor.YELLOW }
    ],
  ...
  var Mangak = class {
    ...
    async getMangaDetails(mangaId) {
      const $2 = await this.fetchCheerio(`${MK_DOMAIN}/${mangaId}`);
      return parseMangaDetails($2, mangaId);
    }
    async getChapters(mangaId) {
      const $2 = await this.fetchCheerio(`${MK_DOMAIN}/${mangaId}`);
      const internalId = parseInternalMangaId($2);
      ...
      return parseChapters(data2, mangaId);
    }
    async getChapterDetails(mangaId, chapterId) {
      const $2 = await this.fetchCheerio(`${MK_DOMAIN}/${mangaId}/${chapterId}`);
      const pages = parseChapterPages($2);
    ...
    async getSearchResults(query, metadata) {
      ...
      const { results, hasNext } = parseSearch(data2);
    ...
    async fetchPopular() {
      const $2 = await this.fetchCheerio(`${MK_DOMAIN}/top/day`);
      return parsePopular($2);
    }
    async fetchLatest(page) {
      const $2 = await this.fetchCheerio(`${MK_DOMAIN}/latest?sort=latest&page=${page}`);
      return parseLatest($2);
    }
