...
var _Sources = (() => {
  ...
  var toPartial = (item, subtitle) => App.createPartialSourceManga({
    mangaId: item.slug,
    title: decode(item.name ?? ""),
    image: item.cover ?? "",
    subtitle
  });
  var latestChapterLabel = (item) => {
    const ch = item.latestChapters?.[0];
    if (!ch) return item.displayUpdatedShort;
    const num = ch.chapter_number ?? ch.slug.split("-")[1];
    return num !== void 0 ? `Ch. ${num}` : ch.name;
  };
  var parseHero = (data) => data.heroItems.map((i) => toPartial(i));
  var parseTrending = (data) => data.trendingItems.map(
    (i) => toPartial(
      i,
      [i.displayRating && `\u2B50 ${i.displayRating}`, i.displayViews && `\u{1F525} ${i.displayViews}`].filter(Boolean).join(" ")
    )
  );
  var parsePopular = (data) => data.popularItems.map((i) => toPartial(i, i.displayUpdatedShort));
  var parseLatestItems = (items) => items.map((i) => toPartial(i, latestChapterLabel(i)));
  var buildHomeSections = () => ({
    hero: {
  ...
  var parseMangaDetails = (manga, mangaId) => {
    const titles = [decode(manga.name)];
    for (const alt of manga.altNames ?? []) {
      const n = decode(alt.name ?? "").trim();
      if (n && !titles.some((t) => t.toLowerCase() === n.toLowerCase())) titles.push(n);
    }
    const tagSections = [];
    const pushTags = (id, label, tags) => {
  ...
  var toChapter = (ch) => {
    ...
    return App.createChapter({
      ...
      chapNum: num,
      time: ts ? new Date(ts) : /* @__PURE__ */ new Date(),
      langCode: "\u{1F1EC}\u{1F1E7}"
    });
  };
  var parseInlineChapters = (chapters) => chapters.map(toChapter);
  var parseApiChapters = (resp) => {
  ...
  var parseSearch = (resp) => {
    ...
    const results = items.filter((i) => i.slug).map((i) => {
      const ch = i.latest_chapters?.[0];
      const subtitle = ch?.chapter_number !== void 0 ? `Ch. ${ch.chapter_number}` : ch?.name ?? i.status;
  ...
  var MangaK = class {
    constructor() {
      ...
      this.sections = buildHomeSections();
    }
    async fetchBuildId() {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const data = await this.fetchNextData("home.json");
    ...
    async getViewMoreItems(homepageSectionId, _metadata) {
      switch (homepageSectionId) {
        case "latest": {
          const data = await this.fetchNextData("latest.json");
        ...
        case "trending": {
          const data = await this.fetchNextData("top/day.json?type=day");
          return App.createPagedResults({
    ...
    async getChapterDetails(mangaId, chapterId) {
      const data = await this.fetchNextData(`${mangaId}/${chapterId}.json`);
